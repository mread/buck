...
def recurse_with_ignore_check(dir, ignore_paths, sought, collector):
  for item in os.listdir(dir):
    next_dir = os.path.join(dir, item)
    if os.path.isdir(next_dir):
      if item not in ignore_paths:
        recurse_with_ignore_check(next_dir, ignore_paths, sought, collector)
    elif item == sought:
      collector.append(next_dir)
...
def get_files_matching(inclusions, exclusions, search_dir, root_dir, ignore_paths, collector):
  for item in os.listdir(search_dir):
    next_dir = os.path.join(search_dir, item)
    if os.path.isdir(next_dir):
      if item not in ignore_paths:
        get_files_matching(inclusions, exclusions, next_dir, root_dir, ignore_paths, collector)
    else:
      relative_path = os.path.join(search_dir, item)[len(root_dir):][1:]
      if passes_glob_filter(relative_path, inclusions, exclusions):
        collector.append(relative_path)


def glob2(includes, excludes=[], build_env=None):
  ...
  parser.add_option('--relative_ignore_path', action='append', dest='relative_ignore_paths')
  ...
  get_files_matching(inclusions, exclusions, search_base, search_base, options.relative_ignore_paths, paths)
...
def java_test(
  ...
  add_rule({
    ...
    'run_from_module_root': run_from_module_root,
    'contacts' : contacts,
    'deps' : deps,
    'visibility' : visibility,
  }, build_env)
...
def main():
  ...
  if args:
    ...
    # pattern matching.
    build_files = []
    recurse_with_ignore_check(project_root, options.relative_ignore_paths,
                                            BUILD_RULES_FILE_NAME, build_files)
